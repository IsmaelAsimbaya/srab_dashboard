...
const UsuarioInfo = () => {
  ...
  const editar = (props) => {
    ...
    navigate(`/actUsuario/${id_usuario}`, {
      state: {
        ci,
        nombres,
        apellidos,
  ...
  const handleDeleteConfirm = () => {
    const id = selectedRow.id_usuario;
    axios
      .delete(`${apiUrl}/usuario/${id}`)
      .then((response) => {
        const updatedRows = rows.filter((r) => r.id !== selectedRow.id);
        setRows(updatedRows);
        setSnackbarMessage("Usuario eliminada exitosamente.");
  ...
  const columns = [
    ...
    {
      field: "id_lugar",
      headerName: "ID Lugar de Trabajo",
      flex: 1,
    },
    {
      field: "id_horario",
    ...
    {
      field: "face_source_url",
      headerName: "URL Video Rostro",
      flex: 1,
    },
    {
      headerName: "Opciones",
  ...
  return (
    <Box m="20px">
      <Header
        title="USUARIOS"
        subtitle="Lista de usuarios registrados en el sistema SRAB."
      />
      <Box
        m="40px 0 0 0"
        height="75vh"
